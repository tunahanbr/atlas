...
export default async function OpenGraphImage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const name = profile?.displayName ?? profile?.user.name ?? profile?.username ?? "Atlas profile";
  return new ImageResponse(
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", background: "#f2eee7", color: "#171714", padding: "72px 82px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 20, color: "#79746a" }}><span>{profile?.location ?? profile?.profileLabel ?? ""}</span><span>atlas.rocks/{profile?.username ?? username}</span></div>
    </div>, size,
  );
}
