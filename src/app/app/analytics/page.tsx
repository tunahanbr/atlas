...
export default async function AnalyticsPage({
  ...
  const hasTraffic = analytics.totals.profileViews > 0;
  const stats = [
  ...
  return (
    <>
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <div key={stat.label} className="rounded-md bg-card/45 p-5">
            <p className="font-editorial text-3xl tabular-nums tracking-[-0.02em]">{stat.value}</p>
      ...
      <section className="mt-6 rounded-md bg-card/45 p-6">
        ...
        {hasTraffic ? (
          ...
          <div className="mt-8 flex h-48 flex-col items-center justify-center rounded-md bg-background/35 px-6 text-center">
            <p className="font-editorial text-lg">The chart begins with your first visitor</p>
            <p className="mt-1 max-w-sm text-sm text-muted-foreground">
              Preview visits count too, so open your public profile once to verify analytics end to end.
            </p>
          </div>
        )}
...
function Metric({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-md bg-card/45 p-5">
